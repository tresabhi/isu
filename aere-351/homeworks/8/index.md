...
And the aiming radius (this uses the hyperbolic eccentricity, not the circular one near Mercury):

$$
a_H' = -\frac{\mu_{mercury}}{v_{\infty,mercury}^2}
$$
...
$$
\Delta = |a_H'| \sqrt{e_H'^2 - 1}
$$

All of that, but in Python:
...
beta = acos(1 / (1 + ((r_p * v_infinity_earth**2) / mu_earth))) * rad
beta = beta.to(deg)

a_H_prime = -mu_mercury / v_infinity_mercury**2
e_H_prime = 1 + r_p_prime / a_H_prime
delta = abs(a_H_prime) * (e_H_prime**2 - 1) ** (1 / 2)
...
```
delta_v (near Earth) = 7.478689597101693 kilometer / second
delta_v (near Mercury) = 9.589190051457704 kilometer / second
delta_v_total = 17.067879648559398 kilometer / second

beta = 89.99730950532013 degree
delta = 57909901.75949829 kilometer
```
